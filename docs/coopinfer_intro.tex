...
\begin{frame}{Evaluator Step 2: Ready-List Event Loop}
  ...
  \end{enumerate}
  Greedy mode commits to one choice per step. Lookahead mode keeps a width-4 beam,
  branches on the first three ready candidates, and prunes partial schedules by a
  serialized-resource completion lower bound.
\end{frame}
...
\begin{frame}{Evaluator Step 3: Ensemble Rules}
  ...
  \end{itemize}
  Greedy mode remains work-conserving through earliest-start selection; lookahead uses the same ordering to choose bounded branch candidates.
\end{frame}
