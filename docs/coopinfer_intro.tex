...
\begin{frame}{Evaluator Step 4: Tail-Latency Variants}
  The evaluator also simulates timing variants that reduce artificial long tail-frame E2E windows:
  \begin{itemize}
    ...
    \item \textbf{Frame-drain gate}: for younger frames, wait for the previous frame output before starting non-source side-branch work.
  \end{itemize}
  These variants avoid counting early work that cannot help output completion as the frame's first non-input event.
  \vspace{0.5em}
  ...
  \[
    2 \times 2 \times 3 = 12
  \]
  deterministic schedules: two transfer timings, two frame-drain modes, and three ready-list rules.
\end{frame}
