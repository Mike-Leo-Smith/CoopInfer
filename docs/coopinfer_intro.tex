...
\begin{frame}{Prototype Loop}
  ...
  \begin{tikzpicture}[
    ...
    \node[block, fill=orange!18, below=1.25cm of graph] (eval) {Native Evaluator\\Schedule Ensemble};
...
\begin{frame}{Evaluator Step 2: Ready-List Event Loop}
  \small
  The simulator maintains pending dependencies, per-task ready times, and one serialized queue clock per resource:
  \[
    S_t=\max(ready_t,\ release_t,\ resource\_ready_t)
  \]
  For source tasks, the resource clock is zero. Device, Host, and Network tasks
  use their serialized queue clocks.

  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.49\textwidth}
      \textbf{Greedy loop}
      \begin{enumerate}
        \item Add zero-pending tasks to ready.
        \item Pick the earliest feasible start.
        \item Use the active rule as tie-breaker.
        \item Record the interval and release successors.
      \end{enumerate}
    \end{column}
    \begin{column}{0.49\textwidth}
      \textbf{Lookahead brancher}
      \begin{itemize}
        \item Sort ready candidates by start, rule priority, and task id.
        \item Branch on the first three candidates.
        \item Keep a width-4 beam using a serialized-resource completion bound.
        \item Score completed schedules with the same global loss.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Evaluator Step 3: Ensemble Rules}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.49\textwidth}
      \textbf{Latency-oriented}
      \begin{itemize}
        \item \textbf{CriticalPath}: rank plus older-frame aging.
        \item \textbf{OutputFirst}: output tasks, older frames, then rank.
        \item \textbf{FifoReady}: stable creation order after feasible start.
      \end{itemize}
    \end{column}
    \begin{column}{0.49\textwidth}
      \textbf{Resource-oriented}
      \begin{itemize}
        \item \textbf{DeviceFirst}: Source, Device, Network, Host.
        \item \textbf{HostFirst}: Source, Host, Network, Device.
        \item \textbf{NetworkFirst}: Source, Network, Device, Host.
        \item \textbf{Throughput}: younger frames before rank.
      \end{itemize}
    \end{column}
  \end{columns}
  \vspace{0.4em}

  Greedy mode uses this ordering only after earliest-start comparison; lookahead
  uses it to choose bounded branch candidates.
\end{frame}
...
\begin{frame}{Evaluator Step 4: Tail-Latency Retiming}
  ...
  \]
  deterministic schedule candidates: two transfer timings, retiming on/off, greedy/lookahead mode, and seven ready-list rules.
\end{frame}
...
\begin{frame}{Evaluator Step 5: Metrics and Winner Selection}
  ...
  \]
  The winner is the schedule with the lowest global split loss used by the solver. Ties prefer lower max-frame latency, then lower average latency, then lower initiation interval, then higher Device utilization.
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
        ...
        \item Config views use a bold outline for fixed-device nodes; solved source events use dashed outlines.
...
\begin{frame}{Topology and Timeline Reading Guide}
  ...
  \begin{tikzpicture}[
    ...
    \node[font=\scriptsize] at (1.8,1.75) {Topology: fill = frame, outline = placement, dashed edge = transfer};
