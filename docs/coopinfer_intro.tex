...
\begin{frame}{Evaluator: Data Dependency + Resource Queues}
  ...
  \[
    ...
    \right)
  \]
  where input/source nodes may use $release_i(f)=phase_i+f\cdot period_i$ for frame $f$.
  Source nodes are zero-duration external events: they do not occupy the device or host worker queues, and later releases may occur while older frames are still being processed downstream.
  \vspace{0.5em}
