...
\begin{frame}{原型闭环}
  ...
  \begin{tikzpicture}[
    ...
    \node[block, fill=orange!18, below=1.25cm of graph] (eval) {原生解析器\\调度集成};
...
\begin{frame}{内层解析器步骤 2：就绪列表事件循环}
  \small
  仿真器维护剩余依赖数、任务就绪时间，以及三类串行资源队列时钟：
  \[
    S_t=\max(ready_t,\ release_t,\ resource\_ready_t)
  \]
  Source 任务的资源时钟为 0；Device、Host、Network 使用各自的串行队列时钟。

  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.49\textwidth}
      \textbf{贪心循环}
      \begin{enumerate}
        \item 零 pending 任务进入 ready。
        \item 选择最早可行开始时间。
        \item 用当前规则打破平局。
        \item 记录区间并释放后继。
      \end{enumerate}
    ...
    \begin{column}{0.49\textwidth}
      \textbf{Lookahead 分支}
      \begin{itemize}
        \item 按开始时间、规则优先级、任务 id 排序。
        \item 对前三个 ready 候选分支。
        \item 用串行资源完成下界保留宽度为 4 的 beam。
        \item 完整调度仍用同一个全局 Loss 评分。
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{内层解析器步骤 3：多规则集成}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.49\textwidth}
      \textbf{时延导向}
      \begin{itemize}
        \item \textbf{CriticalPath}：rank 加老帧加成。
        \item \textbf{OutputFirst}：输出任务、老帧、rank。
        \item \textbf{FifoReady}：可行开始后按创建顺序。
      \end{itemize}
    \end{column}
    \begin{column}{0.49\textwidth}
      \textbf{资源导向}
      \begin{itemize}
        \item \textbf{DeviceFirst}：Source、端侧、Network、边侧。
        \item \textbf{HostFirst}：Source、边侧、Network、端侧。
        \item \textbf{NetworkFirst}：Source、Network、端侧、边侧。
        \item \textbf{Throughput}：年轻帧优先，然后 rank。
      \end{itemize}
    \end{column}
  \end{columns}
  \vspace{0.4em}

  贪心模式只在最早开始时间相同后使用该排序；lookahead 用它选择有限分支候选。
\end{frame}

\begin{frame}{内层解析器步骤 4：尾帧时延重排}
  解析器会降低人为拉长的尾帧 E2E，但不添加跨帧全局门控：
  \begin{itemize}
    \item \textbf{延迟受阻传输}：若跨设备目标还在等待其他前驱，则把传输推迟到这些前驱完成后再启动。
    \item \textbf{右移扫描}：列表调度完成后，把有余量的网络任务和 join 支路计算向后移动，但必须保持后继、资源顺序、释放时间和输出完成时刻不变。
  \end{itemize}
  ...
  \]
  套确定性调度候选：两种传输时序、是否右移重排、贪心/lookahead 模式，以及七种就绪列表规则。
\end{frame}
...
\begin{frame}{内层解析器步骤 5：指标提取与胜者选择}
  ...
  \]
  最终保留与求解器一致的全局切分 Loss 最低的调度；若 Loss 相同，依次选择最大单帧时延更低、平均时延更低、启动间隔更低、端侧利用率更高的方案。
\end{frame}
...
\begin{frame}{求解模式}
  \begin{table}
    ...
    \begin{tabular}{p{0.2\textwidth}p{0.23\textwidth}p{0.46\textwidth}}
      ...
      Enumerate & 小规模 DAG & 将 assignment mask 按线程切分，返回精确全局最优可行解 \\
      Random Search & 较大 DAG & 多线程独立扰动链，保留最佳可行候选 \\
      Simulated Annealing & 跳出局部最优 & 多线程独立退火链，支持配置初始/最终温度 \\
      \bottomrule
    \end{tabular}
  \end{table}
  \vspace{0.5em}
  \texttt{solver\_threads}=0 表示使用硬件线程数。所有模式都会强制固定端侧节点，并执行平均端到端和最大单帧时延上限过滤。
\end{frame}
...
\begin{frame}{GUI 使用流程}
  \begin{enumerate}
    ...
    \item 编辑边和传输数据量。
    \item 配置带宽、固定时延、网络批处理、优化权重、求解模式、线程数、迭代次数、退火温度和时延上限。
    \item 点击 \textbf{Solve}。
    \item 查看时延、资源利用率、拓扑放置和 profiler 风格时序图。
  \end{enumerate}
  \vspace{0.5em}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
        ...
        \item 配置图用加粗轮廓表示固定端侧节点；求解图用虚线轮廓表示输入源事件。
...
\begin{frame}{拓扑图与时序图读法}
  \centering
  \begin{tikzpicture}[
      node/.style={circle, draw=black, line width=1.3pt, minimum size=0.82cm, font=\bfseries\scriptsize},
      arrow/.style={-{Latex[length=2.4mm]}, thick},
      lane/.style={draw=gray!35},
      op/.style={rounded corners=2pt, minimum height=0.32cm, font=\scriptsize},
      x=1cm, y=1cm
    ]
    \node[node, fill=blue!25, draw=blue!70!black] (a) at (0,2.9) {v1};
    \node[node, fill=orange!30, draw=green!60!black] (b) at (1.8,3.35) {v2};
    \node[node, fill=orange!30, draw=green!60!black] (c) at (1.8,2.45) {v3};
    \node[node, fill=blue!25, draw=blue!70!black] (d) at (3.6,2.9) {v4};
    ...
    \node[font=\scriptsize] at (1.8,1.75) {拓扑图：填充表示帧，轮廓表示放置，虚线边表示传输};
    ...
    \begin{scope}[shift={(5.2,2.0)}, x=0.045cm, y=0.7cm]
      ...
      \node[op, fill=blue!25, draw=blue!70!black, minimum width=22*0.045cm] at (11,0.2) {v1};
      \node[op, fill=orange!55, minimum width=52*0.045cm] at (48,1.1) {batch};
      \node[op, fill=orange!30, draw=green!60!black, minimum width=22*0.045cm] at (85,2.0) {v2};
      \node[op, fill=orange!30, draw=green!60!black, minimum width=18*0.045cm] at (105,2.0) {v3};
      \node[op, fill=blue!25, draw=blue!70!black, minimum width=24*0.045cm] at (122,0.2) {v4};
    \end{scope}
