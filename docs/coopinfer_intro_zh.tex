...
\begin{frame}{内层解析器步骤 2：就绪列表事件循环}
  ...
  \end{enumerate}
  贪心模式每步提交一个选择；lookahead 模式保留宽度为 4 的 beam，
  对前三个 ready 候选分支，并用串行资源完成时间下界裁剪部分调度。
\end{frame}
...
\begin{frame}{内层解析器步骤 3：多规则集成}
  ...
  \begin{itemize}
    ...
    \item \textbf{HostFirst}：Source、边侧、Network、端侧依次优先。
    \item \textbf{NetworkFirst}：Source、Network、端侧、边侧依次优先。
    \item \textbf{OutputFirst}：输出计算任务优先，然后按老帧加成和 rank 排序。
    \item \textbf{Throughput}：年轻帧加成和 rank，用于测试稳态流水调度。
    \item \textbf{FifoReady}：在相同可行开始时间下按任务创建顺序稳定排序。
  \end{itemize}
  贪心模式通过最早可行开始时间保持资源不无谓空转；lookahead 使用同一排序来选择有限分支候选。
\end{frame}
