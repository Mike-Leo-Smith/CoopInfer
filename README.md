...
- `latency_limit` is an optional amortized E2E latency cap in milliseconds. `max_frame_latency_limit` is an optional worst-frame E2E cap. Use `0` to disable either limit; scheduler candidates that satisfy positive limits are selected before lower-loss candidates that violate them, and assignments with no satisfying schedule are rejected.
