...
def main() -> None:
    ...
    # Match the real inference path. Keep attention eager so export sees the
    # unfused dependency structure rather than an implementation-specific SDPA kernel.
    ...
    class InferenceOneStepWrapper(nn.Module):
        ...
        def forward(
            ...
            # PI05 only consumes continuous state history here when the native
            # proprioceptive-memory option is enabled. Otherwise robot state is
            # represented upstream by the model's tokenization/preprocessing path.
